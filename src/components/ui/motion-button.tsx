...
const MotionButton = forwardRef<HTMLButtonElement, MotionButtonProps>(function MotionButton(
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        'group relative isolate inline-flex h-12 min-w-[11.25rem] cursor-pointer items-center overflow-hidden rounded-full border p-1 font-geist outline-none transition-[opacity,box-shadow] duration-200',
    ...
    >
      ...
      <span
        aria-hidden="true"
        className={cn(
          'absolute start-1 top-1/2 z-10 grid size-10 -translate-y-1/2 place-items-center rounded-full',
          isPrimary ? 'bg-zinc-950 text-white' : 'bg-white text-zinc-950',
        )}
      >
        <ArrowIcon className="size-[18px]" strokeWidth={1.8} />
      </span>
      <span
        className={cn(
          'relative z-10 mx-auto px-10 text-center text-[13px] font-bold tracking-normal whitespace-nowrap transition-colors duration-[400ms] ease-out motion-reduce:transition-none',
          isPrimary
